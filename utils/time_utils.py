...
lastweek  = (datetime.date.today() - datetime.timedelta(days = ((datetime.date.today().weekday() + config.RESET_DAY_OFFSET) % 7))).strftime('%Y%m%d')
...
def update_time():
    ...
    now_time = time.gmtime(time.time() - 3600 * (config.RESET_HOUR - 1))
    ...
    if now != today:
        ...
        time_left = (config.RESET_HOUR - time.gmtime().tm_hour) % 24
        return False
